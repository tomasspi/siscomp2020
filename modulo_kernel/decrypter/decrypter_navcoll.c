/*  
 *  Módulo de desencriptación. No contiene la funcion write porque solo se utiliza para desencriptar (leer),
 */
...
MODULE_DESCRIPTION("3er trabajo prático - FCEFyN UNC 2018 - Sistemas Operativos 1 // Desencriptador de mensajes"); 
 ...
 */
#define BUF_LEN 100
#define DEVICE_NAME		"decrypter_navcoll"
struct cdev *mcdev;		/* m es 'my' */
...
extern char msj[BUF_LEN]; 	/* se hace uso de la variable importada por el encriptador */
...
int device_open(struct inode *inode, struct file *filp){
	...
	if(down_interruptible(&virtual_device.sem) != 0){
		printk(KERN_ALERT "nav-coll(decrypter): error de exclusión mutua\n");
		return -1;
	}
	
	printk(KERN_INFO "nav-coll(decrypter): dispositivo abierto\n");
	return 0;
}

/*
 *  funcion llamada cuando el usuario quiere extraer información del dispostivo
 */
ssize_t device_read(struct file* filp, char* bufStoreData, size_t bufCount, loff_t* curOffset){
	...
	printk(KERN_INFO "navcoll(decrypter): leyendo del dispositivo\n");
	for(i = 0;i<bufCount && i<BUF_LEN;i++){		/* desencripto el mensaje */
        	msj[i] = msj[i] - 1;
  	}
	ret = copy_to_user(bufStoreData, msj,bufCount);
	return ret;
}
...
int device_close(struct inode *inode, struct file *filp){
	...
	printk(KERN_INFO "navcoll(decrypter): dispositivo cerrado\n");
...
static int driver_entry(void){
	/* TERCERO. Se registra el dispositivo al sistema en dos (2) pasos: */
	...
	if(ret < 0) { /* si retorna un numero negativo hubo un error */
		printk(KERN_ALERT "navcoll(decrypter): falló al alocar major number");
		return ret; /* el error */
	}
	...
	printk(KERN_INFO "navcoll(decrypter): major number es %d. \tusar \"mknod /dev/%s c %d 0\" para archivo de dispositivo\n", major_number, DEVICE_NAME, major_number);
	...
	if(ret < 0) { /* por si hay algun error */
		printk(KERN_ALERT "navcoll(decrypter): error añadiendo cdev al kernel");
		return ret; /* el error */
	}
...
static void driver_exit(void){
	...
	printk(KERN_ALERT "navcoll(decrypter): unloaded module");
}
 ...
 *
 * Luego del '$ make' ejecutar '$ sudo insmod decrypter_navcoll.ko'
 *
 ...
 * 
 * Para descargar el modulo se ejecuta 'sudo rmmod decrypter_navcoll' y 'sudo rm /dev/decrypter_navcoll'
 *
 */
